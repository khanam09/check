  ...
  result <- pop_data %>%
    ...
    dplyr::summarise(
      ...
      infection_deaths = sum(
        as.numeric(!!rlang::sym(infection_flag_col)), na.rm = TRUE
      ),
      .groups = "drop"
    ) %>%
    dplyr::mutate(
...
calculate_Rkd_prime <- function(ast_data,
    ...
    if (!is.null(pathogen_name)) {
        ...
        if (nrow(ast_data) == 0) {
            stop(sprintf("No records found for pathogen(s): %s",
                         paste(pathogen_name, collapse = ", ")))
        }
        message(sprintf("Pathogen filter applied: %s",
                        paste(pathogen_name, collapse = ", ")))
    }
...
calculate_P_Lk_prime <- function(data,
    ...
    required_cols <- c(syndrome_col, specimen_col, polymicrobial_col,
                       patient_col, pathogen_col, outcome_col)
    missing_cols  <- setdiff(required_cols, names(data))
    if (length(missing_cols) > 0) {
        stop(sprintf("Missing column(s) in data: %s",
                     paste(missing_cols, collapse = ", ")))
    }
    if (!is.null(facility_name) && is.null(facility_col)) {
...
calculate_P_Lk_fatal <- function(data,
    ...
    # ── Input validation ───────────────────────────────────────────────────────
    required_cols <- c(syndrome_col, polymicrobial_col,
                       patient_col, pathogen_col, outcome_col)
    ...
    if (!is.null(pathogen_name)) {
        df <- df %>% dplyr::filter(.data[[pathogen_col]] %in% pathogen_name)
        if (nrow(df) == 0) {
            stop(sprintf("No fatal records found for pathogen(s): %s",
                         paste(pathogen_name, collapse = ", ")))
        }
        message(sprintf("Pathogen filter applied: %s",
                        paste(pathogen_name, collapse = ", ")))
...
#' within syndrome L:
#'
#' \deqn{YLD_K = \text{Incidence}_L \times P'_{LK} \times \text{proxy\_yld\_per\_case}}
#'
#' The proxy YLD per incident case is drawn from a GBD-derived reference table
#' stratified by Indian state (\code{yld_ref}). When facility location is known
#' the state-specific value is used; otherwise the India-wide value is applied.
#'
...
#' required so that the correct state-level YLD weight can be applied.
#'
...
#'   \code{proxy_yld_per_case}. Loaded from
#'   \file{inst/extdata/Proxy_YLD_per_case.xlsx}.
#' @param state_name      Character or NULL. State for YLD weight in pooled
#'   mode. NULL uses the India row. Ignored in facility-level mode.
#' @param facility_col    Character or NULL. Facility identifier column.
...
#'   Required in facility-level mode.
...
#'   \code{incidence_L}, \code{proxy_yld_per_case}, \code{YLD},
...
calculate_YLD <- function(incidence_data,
    ...
    # ── Input validation ──────────────────────────────────────────────────────
    if (!all(c("location_name", "proxy_yld_per_case") %in% names(yld_ref))) {
        stop("yld_ref must have columns: 'location_name', 'proxy_yld_per_case'.")
    }
    ...
    if (!is.null(pathogen_name)) {
        ...
        if (nrow(P_Lk_prime_tbl) == 0) {
            stop(sprintf("No rows in P_Lk_prime_tbl for pathogen(s): %s",
                         paste(pathogen_name, collapse = ", ")))
        }
    }
    ...
    if (!pool_by_facility) {
        ...
        if (is.data.frame(incidence_data)) {
            if (!incidence_col %in% names(incidence_data)) {
                stop(sprintf("incidence_col '%s' not found in incidence_data.",
                             incidence_col))
            }
        ...
        result <- P_Lk_prime_tbl %>%
            ...
            dplyr::mutate(
                ...
                proxy_yld_per_case = yld_weight,
                YLD                = incidence_L * .data[[plk_col]] * proxy_yld_per_case
            )

        message(sprintf(
            "YLD computed (pooled, location='%s'): %d pathogen(s), total YLD = %.2f.",
            loc, nrow(result), sum(result$YLD, na.rm = TRUE)
        ))
    ...
    # Join P'LK with incidence and state mapping
    result <- P_Lk_prime_tbl %>%
        ...
        dplyr::left_join(
            yld_ref %>%
                dplyr::select(location_name, proxy_yld_per_case) %>%
    ...
    missing_yld <- result %>%
        dplyr::filter(is.na(proxy_yld_per_case)) %>%
    ...
    if (length(missing_yld) > 0) {
        ...
        result <- result %>%
            dplyr::mutate(
                proxy_yld_per_case = dplyr::if_else(
                    is.na(proxy_yld_per_case), india_yld, proxy_yld_per_case
                )
            )
    }
    ...
    result <- result %>%
        dplyr::mutate(
            YLD = .data[[incidence_col]] * .data[[plk_col]] * proxy_yld_per_case
        )
...
#' Safely fit a distribution, returning NULL on failure
#' @keywords internal
.safe_fitdist <- function(x, dist) {
    if (length(x) < 5L || !requireNamespace("fitdistrplus", quietly = TRUE))
        return(NULL)
    tryCatch(
...
derive_infection_type <- function(
    ...
    if (any(cannot_infer)) {
        ...
        message(sprintf(
            paste0("Cannot infer infection type for %d patient(s): ",
        ...
        if (patient_id_col %in% names(data)) {
            ...
            message("  Row indices with missing date(s): ",
...
compute_patient_los <- function(
    ...
    # ── Column validation ──────────────────────────────────────────────────────
    required <- c(patient_id_col, facility_col, organism_col,
    ...
    episode_min_cult <- data %>%
        ...
        dplyr::ungroup() %>%
        dplyr::select(!!rlang::sym(patient_id_col),
                      !!rlang::sym(date_culture_col),
                      !!rlang::sym(organism_col),
                      .cult_ep_min)
...
fit_los_rr_nima <- function(
    ...
    # ── Resolve pathogens ──────────────────────────────────────────────────────
    pathogens <- if (!is.null(organism_name)) organism_name else
    ...
    .compute_los_estimate <- function(los_vec, dists, method) {
        if (method == "median") {
            ...
            return(list(estimate  = if (is.finite(med)) med else NA_real_,
        ...
        list(estimate  = .compute_mean_from_fit(fits[[best]], best),
             best_dist = best)
    }
    ...
    for (path in pathogens) {
        ...
        model_data <- dplyr::inner_join(
            los_pat, resist_wide, by = patient_id_col
        ) %>%
            dplyr::mutate(
        ...
        if (is.na(est_S_ref$estimate) || est_S_ref$estimate <= 0) {
            reason <- if (is.na(est_S_ref$estimate)) "estimation returned NA (distribution fitting may have failed)"
                      else sprintf("%s LOS estimate is %.4g (must be > 0)", los_summary, est_S_ref$estimate)
            warning(sprintf(
...
fit_los_rr_poisson <- function(
    ...
    .fit_one_class <- function(sub_data, cls_col, fac_col, use_fac_fe) {
        ...
        if (length(unique(stats::na.omit(vals))) < 2L) return(NULL)
        if (nrow(sub_data) < min_n) return(NULL)
        fmla <- if (use_fac_fe &&
    ...
    .extract_rr <- function(model, cls_col) {
        ...
        if (length(cname) == 0L) return(NULL)
        beta <- coefs[[cname[1L]]]
        se   <- sqrt(vcmat[cname[1L], cname[1L]])
        data.frame(RR       = exp(beta),
    ...
    for (path in pathogens) {
        ...
        model_data <- dplyr::inner_join(
            los_data, resist_wide, by = patient_id_col
        ) %>%
            dplyr::mutate(
        ...
        if (centre_model == "pooled_fe") {
            ...
            for (cls in class_safe) {
                ...
                for (ctr in centres) {
                    ctr_df <- dplyr::filter(model_data,
...
fit_los_rr_distribution <- function(
    ...
    if (nrow(df) == 0L) {
        warning(sprintf(
            "No discharged patients remain after filters (outcome='%s'%s). Returning empty data frame.",
            final_outcome_value,
            if (!is.null(syndrome_name)) sprintf(", syndrome='%s'", syndrome_name) else ""
        ))
        return(data.frame())
    ...
    .fit_dist_mean <- function(los_vec) {
        fits <- Filter(Negate(is.null),
                       setNames(lapply(distributions, .safe_fitdist, x = los_vec),
                                distributions))
        if (length(fits) == 0L)
...
assign_rr_to_profiles <- function(
    ...
    if (length(missing_rr) > 0L)
        stop(sprintf("Column(s) not found in rr_table: %s",
    ...
    for (path in names(profiles_output)) {
        ...
        for (i in seq_len(n_prof)) {
            ...
            if (length(resist_cls) == 0L) {
                ...
                if (has_ci) { ci_lo_prof[i] <- 1.0; ci_hi_prof[i] <- 1.0 }
                next
            }
...
filter_profiles_to_rr_classes <- function(
    ...
    if (!all(c(pathogen_col, class_col) %in% names(rr_table)))
        stop(sprintf("Columns '%s' and '%s' must be present in rr_table.",
    ...
    for (path in names(profiles_with_rr)) {
        ...
        non_class_cols <- c("profile", probability_col, "RR_LOS_profile",
...
compute_paf_los <- function(
    ...
    for (path in names(profiles_with_rr)) {
        ...
        for (col in c(probability_col, rr_profile_col, profile_col)) {
            if (!col %in% names(df))
                stop(sprintf("Column '%s' not found in profiles for '%s'.",
                             col, path))
        }
    ...
    for (path in names(profiles_with_rr)) {
        ...
        if (length(class_cols) > 0L) {
            is_resistant <- rowSums(
                df[, class_cols, drop = FALSE] == 1L, na.rm = TRUE
            ) > 0L
        } else {
...
#' burden fraction (from \code{compute_fraction_associated()}).
#'
#'   YLD_associated_k = YLD_k * Fraction_k
#'
#' Answers: "How much disability burden occurs among patients with resistant
#' infections?"  This is a burden partition — it includes the baseline
#' disability that would have occurred even in susceptible infections.
#'
...
#'   \code{E_RR_k}, and \code{YLD_associated}.
#' @export
...
derive_infection_type_for_mortality <- function(
    ...
    # ── Column validation ──────────────────────────────────────────────────────
    required <- c(infection_type_col, date_admission_col,
                  date_culture_col, final_outcome_col)
    missing_cols <- setdiff(required, names(data))
    if (length(missing_cols) > 0L)
        stop(sprintf("Column(s) not found in data: %s",
    ...
    if (has_outcome_date) {
        ...
        if (any(dead_no_date)) {
            ...
            message(sprintf(
                paste0("[Mortality] Data-quality flag: %d patient row(s) have ",
    ...
    if (any(cannot_infer)) {
        n_cannot <- sum(cannot_infer)
        message(sprintf(
            paste0("[Mortality] Cannot infer HAI/CAI for %d patient row(s): ",
    ...
    data <- data %>%
        dplyr::mutate(
            ...
            .cannot_infer = (.inf_raw %in% c("NOT KNOWN", "NOT_KNOWN",
                                              "UNKNOWN", "NULL", "NA", "") |
                                 is.na(.data[[infection_type_col]])),
            infection_type_derived = dplyr::case_when(
                # ── Explicit HAI labels ────────────────────────────────────────
...
.derive_icu_binary <- function(
    ...
    icu_values        = c("ICU", "Intensive Care", "Critical Care",
                          "PICU", "NICU"),
    ...
    if (prop_missing > missing_threshold) {
        message(sprintf(
            paste0("[ICU] %.1f%% of patients have entirely missing unit_type. ",
...
.encode_comorbidity_mortality <- function(
    ...
    ord_severe   <- c("SEVERE", "HIGH", "MAJOR", "HEAVY",
    ...
    message(sprintf(
        ...
        paste(
            paste0(levels(data$comorbidity_encoded), " = ",
...
.check_hai_icu_collinearity <- function(df,
    ...
    if (!is.na(phi) && abs(phi) >= phi_threshold) {
        warning(sprintf(
            paste0("[Collinearity] HAI and ICU are highly correlated ",
                   "(phi = %.3f \u2265 threshold %.2f). ",
...
fit_mortality_rr_logistic <- function(
    ...
    if (use_syndrome_covariate) {
        ...
        patient_covars <- dplyr::left_join(
            patient_covars, syndrome_raw, by = patient_id_col
        )
        message(sprintf(
    ...
    for (path in pathogens) {
        ...
        if (use_icu)
            invisible(.check_hai_icu_collinearity(
                model_data, hai_col = "HAI", icu_col = "ICU",
        ...
        for (cls in class_safe) {
            ...
            if (requireNamespace("car", quietly = TRUE)) {
                ...
                if (!is.null(vif_vals)) {
                    vif_num  <- if (is.matrix(vif_vals)) vif_vals[, "GVIF"] else
                        as.numeric(vif_vals)
            ...
            all_or[[length(all_or) + 1L]] <- data.frame(
                ...
                syndrome_scope       = if (!is.null(syndrome_name)) syndrome_name else
                    if (use_syndrome_covariate) "covariate" else "all",
...
load_india_life_expectancy <- function(le_path) {
    ...
    rbind(
        data.frame(sex = "Combined", age_bin = age_bins,
                   life_expectancy = parse_le(3:21),  stringsAsFactors = FALSE),
        data.frame(sex = "Male",     age_bin = age_bins,
                   life_expectancy = parse_le(23:41), stringsAsFactors = FALSE),
        data.frame(sex = "Female",   age_bin = age_bins,
...
compute_yll_associated <- function(
    ...
    # ── Input validation ───────────────────────────────────────────────────────
    required_cols <- c(outcome_col, pathogen_col, patient_col,
                       age_bin_col, sex_col)
    ...
    if (!is.null(facility_col)) {
        ...
        message(sprintf("Computing polymicrobial weights for %d facility/facilities...",
                        length(facilities)))
    ...
    if (length(age_bin_map) > 0L)
        df[[age_bin_col]] <- dplyr::recode(as.character(df[[age_bin_col]]),
                                           !!!age_bin_map)
...
#' profiles_or <- assign_rr_to_profiles(profiles_out, rr_table = mort_or,
#'                                      rr_col = "OR_death")
...
compute_yll_attributable <- function(
    ...
    # ── Input validation ───────────────────────────────────────────────────────
    required_cols <- c("yll_contribution", "life_expectancy", "death_weight",
    ...
    if (length(missing_cols) > 0L)
        stop(sprintf("Missing column(s) in yll_patient_data: %s",
                     paste(missing_cols, collapse = ", ")))
